...
typedef struct s_block
{
	...
	#if SHOW_MORE
	size_t	user_size;	/* Size requested by user (only for allocated blocks) */
	#endif
...
# define FT_MIN_BLOCK_SIZE (FT_BLOCK_HDR_SIZE)
...
*/
static inline int	ft_block_is_valid(const ft_block_t *hdr)
{
...
#if SHOW_MORE
ft_block_t	*ft_block_init(void *addr, size_t size, size_t user_size);
#else
ft_block_t	*ft_block_init(void *addr, size_t size);
#endif
...
#if SHOW_MORE
ft_block_t	*ft_block_split(ft_block_t *block, size_t size, size_t user_size);
#else
